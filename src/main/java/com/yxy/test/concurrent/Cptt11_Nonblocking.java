package com.yxy.test.concurrent;
...
//			} catch (InterruptedException e) { }
	...
	static class MyLinked_3<T> implements MyLinkedInterface<T>{
		...
		private final AtomicReference<Node<T>> head
				=new AtomicReference<Node<T>>(new MyLinked_3.Node<T>(null,null));
		...
		public void add(T element) {
			MyLinked_3.Node<T> newNode=new MyLinked_3.Node<T>(element,null);
			while(true){
...
//							} catch (InterruptedException e) { }
		...
		public void displayElement() {
			if(head.get().next!=null){
				MyLinked_3.Node<T> current=head.get().next.get();
				StringBuilder sb=new StringBuilder();
				int count=0;
